...
public class ItemCardControllerTest {
    ...
    public void getTest() throws Exception {
        ...
        mvc.perform(get("/api/itemcard")
                ...
                .andExpect(
                        jsonPath("$[1].make.id", Matchers.equalTo(itemCardList.get(1).getMake().getId().toString())));
    }
    ...
    public void update() throws Exception {
        ...
        Mockito.when(itemCardService.update(ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(itemCard);

        mvc.perform(put("/api/itemcard/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id", Matchers.equalTo(itemCard.getId().toString())))
                .andExpect(jsonPath("$.make.id", Matchers.equalTo(itemCardList.get(0).getMake().getId().toString())))
                .andExpect(jsonPath("$[1].id", Matchers.equalTo(itemCardList.get(1).getId().toString())))
                .andExpect(
                        jsonPath("$[1].make.id", Matchers.equalTo(itemCardList.get(1).getMake().getId().toString())));
    }
