...
export const get = createAsyncThunk<employeeLoanType[]>('employeeLoan/get', async () => {
    return await getLoans();
})
export const getByEmployeeId = createAsyncThunk<employeeLoanType[], string>('employeeLoan/getByEmployeeId', async (employeeId) => {
    return await getEmployeeLoans(employeeId);
})
export const create = createAsyncThunk<employeeLoanType, employeeLoanObjectType>('employeeLoan/create', async (data) => {
    return await createEmployeeLoan(data);
})
export const updateStatus = createAsyncThunk<employeeLoanType, string>('employeeLoan/markCompleted', async (id) => {
...
const employeeLoanSlice = createSlice({
    ...
    extraReducers(builder) {
        ...
        builder.addCase(get.fulfilled, employeeLoanAdapter.setAll)
        builder.addCase(getByEmployeeId.fulfilled, employeeLoanAdapter.setMany)
        builder.addCase(create.fulfilled, employeeLoanAdapter.setOne)
        builder.addCase(updateStatus.fulfilled, employeeLoanAdapter.setOne)
    }
})

export default employeeLoanSlice.reducer

export const { selectAll: selectAllEmployeeLoan, selectById: selectEmployeeLoanById } =
...
export const selectByEmployee = createSelector(
    ...
    (employeeLoans, employeeId) => employeeLoans.filter(loan => loan.employee === employeeId)
)

export const employeeLoanStatus = (state: RootState) => state.employeeLoan.status
export const employeeError = (state: RootState) => state.employeeLoan.error